...
# Importando controllers para el módulo de login
...
def inicio():
    if 'conectado' in session:
        return render_template('base.html', dataLogin=dataLoginSessions())
    else:
...
def perfil():
    if 'conectado' in session:
        return render_template('perfil/perfil.html', info_perfil_session=info_perfil_session())
    else:
...
def loginCliente():
    if 'conectado' in session:
        ...
        if request.method == 'POST' and 'email' in request.form and 'password' in request.form:
            ...
            flash ('el usuario debes iniciar sesion.','error')
            return render_template(f'{PATH_URL_LOGIN}/base_login.html')
